...
- 2026-07-03 — Marp slide export uses a puppeteer-managed Chrome binary (via `npx puppeteer browsers install chrome`), tracked in `.local/chrome-path` (gitignored) rather than the system Firefox or an apt/snap Chromium — Ubuntu 24.04 only ships Chromium as a snap, and marp-cli's Firefox launcher hung on this box. `make preflight` installs it; `make slides` reads the path. — DEMO.md slide-render fix — [open]
